...
class TestExporter:
    ...
    def test_reject_inconsistent_revision_version(self):
        ...
        dataset = finalize_sentence_dataset(table, input_dataset_revision="r1", pipeline_version="v1")
        ...
        dataset_inc = finalize_sentence_dataset(table_inconsistent, input_dataset_revision="r1", pipeline_version="v1")
    ...
    def test_successful_write_read_round_trip(self):
        ...
        dataset = finalize_sentence_dataset(table, input_dataset_revision="rev-1", pipeline_version="ver-1")
    ...
    def test_exact_schema_preservation(self):
        row = make_segmented_row()
        dataset = finalize_sentence_dataset(rows_to_table([row]), input_dataset_revision="r1", pipeline_version="v1")
    ...
    def test_correct_manifest_aggregation(self):
        ...
        rows = [
            make_segmented_row(source="wikipedia", language="en", region="reg-1", sentence_text_normalized="dup"),
            make_segmented_row(source="wikipedia", language="en", region="reg-1", sentence_text_normalized="dup"),
            make_segmented_row(source="wikivoyage", language="fr", region="reg-2", sentence_text_normalized="other"),
        ]
        dataset = finalize_sentence_dataset(rows_to_table(rows), input_dataset_revision="rev-xyz", pipeline_version="ver-123")
    ...
    def test_deterministic_manifest_across_repeated_writes(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir_a, tempfile.TemporaryDirectory() as tmpdir_b:
            ...
            with open(res_a.manifest_path, "r", encoding="utf-8") as fa:
                content_a = fa.read()
            with open(res_b.manifest_path, "r", encoding="utf-8") as fb:
                content_b = fb.read()
    ...
    def test_empty_finalized_dataset(self):
        table = rows_to_table([])
        dataset = finalize_sentence_dataset(table, input_dataset_revision="rev-empty", pipeline_version="ver-empty")
    ...
    def test_failed_replacement_preserves_previous_output(self, monkeypatch):
        ...
        dataset_prev = finalize_sentence_dataset(rows_to_table([row_prev]), input_dataset_revision="r1", pipeline_version="v1")
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            dataset_new = finalize_sentence_dataset(rows_to_table([row_new]), input_dataset_revision="r1", pipeline_version="v1")
    ...
    def test_failed_replacement_and_failed_restoration_preserves_backup(self, monkeypatch):
        row_prev = make_segmented_row(sentence_text_raw="Previous raw text")
        dataset_prev = finalize_sentence_dataset(rows_to_table([row_prev]), input_dataset_revision="r1", pipeline_version="v1")
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            backup_folders = [parent_dir / name for name in os.listdir(parent_dir) if name.startswith(".backup_")]
