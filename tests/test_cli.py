...
class TestCLIHubInput:
    def _fake_result(self) -> PipelineResult:
        fake_export = ExportResult(
            parquet_path=Path("/tmp/out/sentences.parquet"),
            manifest_path=Path("/tmp/out/manifest.json"),
            manifest_data={},
    ...
    def test_both_input_modes_rejected(self, capsys):
        ...
        assert code == 2
        captured = capsys.readouterr()
        assert "mutually exclusive" in captured.err.lower() or "not allowed" in captured.err.lower()
    ...
    def test_missing_both_input_modes_rejected(self, capsys):
        ...
        assert code == 2
        captured = capsys.readouterr()
        assert "input-root" in captured.err.lower() or "input-dataset-id" in captured.err.lower()

    def test_acquisition_failure_nonzero_no_json_no_segmenter(self, capsys, monkeypatch):
        # Acquisition failure: non-zero, concise stderr, no success JSON,
        # and the segmenter is never constructed.
        monkeypatch.setattr("osm_polygon_sentence_relevance.cli.run_pipeline", lambda *a, **k: self._fake_result())
        ...
        code = main([
            ...
            "--input-dataset-revision", "main",
            "--pipeline-version", "ver",
        ], acquisition_fn=mock_acquire, model_factory=mock_factory)
