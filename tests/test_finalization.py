...
def make_segmented_row(
    *,
    ...
    section_path=None,
    sentence_index=0,
    sentence_text_raw="Raw text.",
    sentence_text_normalized="normalized text",
...
class TestFinalization:
    def test_known_sha256_vectors(self):
        ...
        assert sent_id == "9ea8b47c9a8014abeb23d42ba4438524df55fb29b316c36e4e820ae39314a1f3"
    ...
    def test_previous_next_within_one_section(self):
        ...
        rows = [
            make_segmented_row(sentence_index=2, sentence_text_normalized="sentence three"),
            make_segmented_row(sentence_index=0, sentence_text_normalized="sentence one"),
            make_segmented_row(sentence_index=1, sentence_text_normalized="sentence two"),
        ]
        table = rows_to_table(rows)
        res = finalize_sentence_dataset(table, input_dataset_revision="r1", pipeline_version="v1")
    ...
    def test_context_boundary_isolation(self):
        ...
        rows = [
            ...
            make_segmented_row(polygon_id="poly-1", source="wikipedia", document_id="doc-1", section_id="sec-1", sentence_index=0, sentence_text_normalized="s1"),
            # Group 2: poly-2 (different polygon)
            make_segmented_row(polygon_id="poly-2", source="wikipedia", document_id="doc-1", section_id="sec-1", sentence_index=1, sentence_text_normalized="s2"),
            # Group 3: wikivoyage (different source)
            make_segmented_row(polygon_id="poly-1", source="wikivoyage", document_id="doc-1", section_id="sec-1", sentence_index=1, sentence_text_normalized="s3"),
            # Group 4: doc-2 (different document)
            make_segmented_row(polygon_id="poly-1", source="wikipedia", document_id="doc-2", section_id="sec-1", sentence_index=1, sentence_text_normalized="s4"),
            # Group 5: sec-2 (different section)
            make_segmented_row(polygon_id="poly-1", source="wikipedia", document_id="doc-1", section_id="sec-2", sentence_index=1, sentence_text_normalized="s5"),
        ]
        table = rows_to_table(rows)
        res = finalize_sentence_dataset(table, input_dataset_revision="r1", pipeline_version="v1")
        res_rows = res.table.to_pylist()
    ...
    def test_same_source_duplicate_collapse(self):
        ...
        rows = [
            make_segmented_row(document_id="doc-1", section_id="sec-1", sentence_index=0, sentence_text_normalized="dup"),
            make_segmented_row(document_id="doc-2", section_id="sec-2", sentence_index=1, sentence_text_normalized="dup"),
        ]
        table = rows_to_table(rows)
        res = finalize_sentence_dataset(table, input_dataset_revision="r1", pipeline_version="v1")
    ...
    def test_cross_source_duplicate_collapse(self):
        ...
        rows = [
            make_segmented_row(source="wikipedia", document_id="doc-wp", sentence_text_normalized="shared"),
            make_segmented_row(source="wikivoyage", document_id="doc-wv", sentence_text_normalized="shared"),
        ]
        table = rows_to_table(rows)
        res = finalize_sentence_dataset(table, input_dataset_revision="r1", pipeline_version="v1")
        assert res.table.num_rows == 1
    ...
    def test_wikipedia_canonical_preference(self):
        ...
        rows = [
            make_segmented_row(source="wikivoyage", document_id="doc-a", sentence_text_normalized="dup"),
            make_segmented_row(source="wikipedia", document_id="doc-b", sentence_text_normalized="dup"),
        ]
        table = rows_to_table(rows)
        res = finalize_sentence_dataset(table, input_dataset_revision="r1", pipeline_version="v1")
    ...
    def test_deterministic_canonical_tie_breaking(self):
        ...
        res = finalize_sentence_dataset(rows_to_table(rows), input_dataset_revision="r1", pipeline_version="v1")
        assert res.table.to_pylist()[0]["document_id"] == "doc-a"

        # 2. section_index tie break (ascending)
        rows = [
            make_segmented_row(document_id="doc-a", section_index=10, sentence_text_normalized="dup"),
            make_segmented_row(document_id="doc-a", section_index=5, sentence_text_normalized="dup"),
        ]
        res = finalize_sentence_dataset(rows_to_table(rows), input_dataset_revision="r1", pipeline_version="v1")
        ...
        rows = [
            make_segmented_row(document_id="doc-a", section_index=5, section_id="sec-y", sentence_text_normalized="dup"),
            make_segmented_row(document_id="doc-a", section_index=5, section_id="sec-x", sentence_text_normalized="dup"),
        ]
        res = finalize_sentence_dataset(rows_to_table(rows), input_dataset_revision="r1", pipeline_version="v1")
        ...
        rows = [
            make_segmented_row(document_id="doc-a", section_index=5, section_id="sec-x", sentence_index=3, sentence_text_normalized="dup"),
            make_segmented_row(document_id="doc-a", section_index=5, section_id="sec-x", sentence_index=1, sentence_text_normalized="dup"),
        ]
        res = finalize_sentence_dataset(rows_to_table(rows), input_dataset_revision="r1", pipeline_version="v1")
    ...
    def test_raw_text_differences_with_identical_normalized_text_collapse(self):
        ...
        rows = [
            make_segmented_row(sentence_text_raw="Raw A", sentence_text_normalized="normalized"),
            make_segmented_row(sentence_text_raw="Raw B", sentence_text_normalized="normalized"),
        ]
        res = finalize_sentence_dataset(rows_to_table(rows), input_dataset_revision="r1", pipeline_version="v1")
        assert res.table.num_rows == 1
    ...
    def test_shuffle_invariant_table_and_report(self):
        ...
        row1 = make_segmented_row(polygon_id="poly-1", language="en", sentence_text_normalized="one")
        row2 = make_segmented_row(polygon_id="poly-1", language="fr", sentence_text_normalized="two")
        row3 = make_segmented_row(polygon_id="poly-2", language="en", sentence_text_normalized="three")

        table_a = rows_to_table([row1, row2, row3])
        table_b = rows_to_table([row3, row1, row2])

        res_a = finalize_sentence_dataset(table_a, input_dataset_revision="r1", pipeline_version="v1")
        res_b = finalize_sentence_dataset(table_b, input_dataset_revision="r1", pipeline_version="v1")
    ...
    def test_exact_output_schema(self):
        ...
        for name in OUTPUT_SENTENCE_SCHEMA.names:
            ...
            assert expected_field.nullable == actual_field.nullable, f"{name} nullability"

    def test_revision_version_fields(self):
        rows = [make_segmented_row()]
        res = finalize_sentence_dataset(rows_to_table(rows), input_dataset_revision="test-rev", pipeline_version="test-ver")
        row = res.table.to_pylist()[0]
    ...
    def test_blank_configuration_rejection(self):
        ...
        with pytest.raises(FinalizationError):
            finalize_sentence_dataset(table, input_dataset_revision="", pipeline_version="v1")
        with pytest.raises(FinalizationError):
            finalize_sentence_dataset(table, input_dataset_revision="  ", pipeline_version="v1")
        with pytest.raises(FinalizationError):
            finalize_sentence_dataset(table, input_dataset_revision="r1", pipeline_version="")
        with pytest.raises(FinalizationError):
            finalize_sentence_dataset(table, input_dataset_revision="r1", pipeline_version="   ")
    ...
    def test_revision_version_type_rejection(self):
        ...
        # non-string revision
        with pytest.raises(FinalizationError):
            finalize_sentence_dataset(table, input_dataset_revision=123, pipeline_version="v1")
        # non-string version
        with pytest.raises(FinalizationError):
            finalize_sentence_dataset(table, input_dataset_revision="r1", pipeline_version=True)
        # None version/revision
        with pytest.raises(FinalizationError):
            finalize_sentence_dataset(table, input_dataset_revision=None, pipeline_version="v1")
        with pytest.raises(FinalizationError):
            finalize_sentence_dataset(table, input_dataset_revision="r1", pipeline_version=None)
    ...
    def test_invalid_sources_rejected(self):
        ...
        table = rows_to_table(rows)
        with pytest.raises(FinalizationError):
            finalize_sentence_dataset(table, input_dataset_revision="r1", pipeline_version="v1")
    ...
    def test_deterministic_tie_breaking_order_independence(self):
        ...
        res_1 = finalize_sentence_dataset(table_1, input_dataset_revision="r1", pipeline_version="v1")
        res_2 = finalize_sentence_dataset(table_2, input_dataset_revision="r1", pipeline_version="v1")
    ...
    def test_deterministic_tie_breaking_nullable_fields(self):
        ...
        row_art_none = make_segmented_row(article_id=None, sentence_text_normalized="dup")
        row_art_str = make_segmented_row(article_id="art-xyz", sentence_text_normalized="dup")
        table_1a = rows_to_table([row_art_none, row_art_str])
        table_1b = rows_to_table([row_art_str, row_art_none])
        res_1a = finalize_sentence_dataset(table_1a, input_dataset_revision="r1", pipeline_version="v1")
        res_1b = finalize_sentence_dataset(table_1b, input_dataset_revision="r1", pipeline_version="v1")
        assert res_1a.table.equals(res_1b.table)

        # 2. polygon_name (None vs string)
        row_poly_none = make_segmented_row(polygon_name=None, sentence_text_normalized="dup")
        row_poly_str = make_segmented_row(polygon_name="Poly ABC", sentence_text_normalized="dup")
        table_2a = rows_to_table([row_poly_none, row_poly_str])
        table_2b = rows_to_table([row_poly_str, row_poly_none])
        res_2a = finalize_sentence_dataset(table_2a, input_dataset_revision="r1", pipeline_version="v1")
        res_2b = finalize_sentence_dataset(table_2b, input_dataset_revision="r1", pipeline_version="v1")
        assert res_2a.table.equals(res_2b.table)

        # 3. lat/lon (None vs float)
        row_lat_none = make_segmented_row(lat=None, lon=None, sentence_text_normalized="dup")
        row_lat_float = make_segmented_row(lat=34.5, lon=69.1, sentence_text_normalized="dup")
        table_3a = rows_to_table([row_lat_none, row_lat_float])
        table_3b = rows_to_table([row_lat_float, row_lat_none])
        res_3a = finalize_sentence_dataset(table_3a, input_dataset_revision="r1", pipeline_version="v1")
        res_3b = finalize_sentence_dataset(table_3b, input_dataset_revision="r1", pipeline_version="v1")
        assert res_3a.table.equals(res_3b.table)


    def test_empty_input(self):
        empty_table = rows_to_table([])
        res = finalize_sentence_dataset(empty_table, input_dataset_revision="r1", pipeline_version="v1")
    ...
    def test_report_arithmetic(self):
        ...
        rows = [
            make_segmented_row(polygon_id="poly-1", language="en", sentence_text_normalized="normalized_a", source="wikipedia", document_id="doc-1", section_id="sec-1"),
            make_segmented_row(polygon_id="poly-1", language="en", sentence_text_normalized="normalized_a", source="wikivoyage", document_id="doc-2", section_id="sec-2"),
            make_segmented_row(polygon_id="poly-1", language="en", sentence_text_normalized="normalized_b", source="wikipedia", document_id="doc-1", section_id="sec-1"),
            make_segmented_row(polygon_id="poly-1", language="fr", sentence_text_normalized="normalized_a", source="wikipedia", document_id="doc-1", section_id="sec-1"),
        ]
        table = rows_to_table(rows)
        res = finalize_sentence_dataset(table, input_dataset_revision="r1", pipeline_version="v1")
    ...
    def test_synthetic_end_to_end_flow(self):
        ...
        row1 = make_joined_row(polygon_id="poly-1", source="wikipedia", section_text_raw="Hello world. Welcome here.")
        row2 = make_joined_row(polygon_id="poly-1", source="wikivoyage", section_text_raw="Hello world. Travel guide.")
