...
GITHUB_REPO = "https://github.com/NoeFlandre/osm-polygon-wikidata-sentence-relevance.git"
HUGGINGFACE_DATASET = "https://huggingface.co/datasets/NoeFlandre/osm-polygon-wikidata-sentence-relevance"

# Default external storage location requested by the user
DEFAULT_LOCAL_DATA_DIR = Path("/Volumes/Seagate M3/projects/osm-polygon-wikidata-sentence-relevance")
...
def get_data_dir() -> Path:
    ...
    if env_dir:
        path = Path(env_dir)
        if not path.exists():
            print(f"Warning: OSM_DATA_DIR is set to '{path}' but it does not exist.", file=sys.stderr)
        return path
...
if __name__ == "__main__":
    ...
    print(f"Data directory status: {'EXISTS' if resolved_path.exists() else 'NOT FOUND'}")
